with safe_transfers AS (
    ...
    where block_date >= date('2023-01-01') AND "from" IN (SELECT address from safe_ethereum.safes) AND amount_usd > 0
),
receiver_labels AS (
    select 
        t.blockchain,
    ...
    from safe_transfers t
        ...
        LEFT JOIN labels.contracts lc ON t.to = lc.address AND t.blockchain = lc.blockchain
        LEFT JOIN labels.addresses la ON t.to = la.address AND t.blockchain = la.blockchain
...
evt_names AS (
    SELECT DISTINCT tx_hash, index, event_name
    FROM ethereum.logs_decoded
    WHERE tx_hash IN (SELECT tx_hash FROM safe_transfers)
),
...
labelled_transfers AS (
    SELECT
        ...
        t.block_time,
        t.tx_hash,
        CASE WHEN stb.stablecoin_address IS NOT NULL THEN 'stablecoin' ELSE token_standard END AS token_category,
        "from" AS safe_sender,
        to AS receiver,
        CASE 
            WHEN to IN (SELECT address FROM safe_ethereum.safes) THEN 'Safe'
            WHEN to IN (SELECT address FROM labels.burn_addresses) THEN 'Burner address'
            WHEN to IN (SELECT address FROM ethereum.creation_traces) THEN 'Smart contract'
        ...
        amount_usd
    FROM safe_transfers t
        LEFT JOIN dune.safe.dataset_stablecoin_addresses_by_chain stb ON t.blockchain = stb.blockchain AND t.contract_address = stb.stablecoin_address
        LEFT JOIN receiver_labels l ON t.to = l.address
        LEFT JOIN evt_names e ON t.tx_hash = e.tx_hash AND t.evt_index = e.index
